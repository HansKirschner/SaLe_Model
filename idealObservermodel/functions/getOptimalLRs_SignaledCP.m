function output=getOptimalLRs_SignaledCP(input)
...
%     pS=[ones(size(p)).*(1-input.H), ones(size(p)).*(input.H)];
    p=p.*(1-input.H(i))+cpPrior.*(input.H(i)); % actually, this is multiplying pByS by pS and integrating over CP dimension... so pS is never used... 
