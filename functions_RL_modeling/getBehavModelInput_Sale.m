function input = getBehavModelInput_Sale(datastruct,paramsToFit,paramValue)
% keyboard
...
input.feedConfir = ones(length(datastruct.flash),1);
input.feedConfir(datastruct.event==2|datastruct.event==3) = 0;

input.usePrior      = true; % - here we do the "poor man's method of hiracical fittting", which works decently well
